   ...
   账户的业务层实现类
         ...
         曾经xml的配置
          ...
          用于注入数据的
               ...
               @Autowrite:
                    作用：自动安装类型注入。只要容器有唯一的一个bean对象类型和药注入的变量类型匹配，就可注入成功
                    出现位置：
                        可以是变量上，也可以是方法上
                    细节：
                        在使用注解注入时，set方法就不是必须的
               @Qualifier: (不能独自使用，必须和@autowrite一起使用)
                    作用：在安装类中注入的基础之上再安装名称注入。它在给类成员注入时不能单独使用。但是在给方法参数注入时可以
                    属性：
                        value:用于指定注入bean的id
                @Resource
                    作用：直接安装bean的id注入。他可以独立使用
                    属性：
                        name：用于指定bean的id
          ...
          用于改变作用范围的
               他们的作用就和xml配置文件中的bean标签中使用scope属性的功能是一样的
               Scope
                    作用：
                        用于指定bean的作用范围
                    属性：
          ...
          和声明周期相关的
               他们的作用就和xml配置文件中的bean标签中使用init-method和destroy-method的作用是一样的
               PreDestroy
                    作用：用于指定销毁方法
